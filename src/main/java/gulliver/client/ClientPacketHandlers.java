    ...
    public static void register() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(Payloads.AttachEntitySpecial.TYPE, (payload, ctx) -> {
            ctx.client().execute(() -> {
                ...
                gulliver.mixin.iface.IGulliverShoulderInternal carrierAccess =
                        (gulliver.mixin.iface.IGulliverShoulderInternal) carrier;
                if (payload.attachmentType() == 0) {
                    ...
                    if (passenger != null) {
                        ((gulliver.mixin.iface.IGulliverShoulderInternal) passenger).gulliver$setHoldingEntity(null);
                    } else if (prev != null) {
                        // Passenger entity is gone — best-effort detach.
                    }
                } else if (passenger != null) {
                    carrierAccess.gulliver$setHeldEntity(passenger.getUUID());
                    ((gulliver.mixin.iface.IGulliverShoulderInternal) passenger)
